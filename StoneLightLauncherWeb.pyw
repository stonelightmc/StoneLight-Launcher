...
def _serve_browser_fallback(root: Path, config: dict, reason: str = "") -> int:
    ...
    class Handler(BaseHTTPRequestHandler):
        server_version = "StoneLightLauncherBrowserFallback/0.6.67"
    ...
    url = f"http://127.0.0.1:{port}/web_ui/index.html?v={config.get('launcher_version', '0.6.67')}&transport=browser#desktop=1"
...
def _run_webview(root: Path, config: dict, icon_path: Path | None) -> None:
    ...
    os.chdir(root)
    desktop_url = "web_ui/index.html?v=0.6.67#desktop=1"

    api = LauncherWebAPI()
    window_kwargs = {
        "title": f"StoneLight Launcher v{config.get('launcher_version', '0.6.67')}",
        "url": desktop_url,
    ...
    webview.start(
        ...
        private_mode=False,
        http_server=True,
        storage_path=str(root / "data" / "webview" / "0_6_67"),
    )
