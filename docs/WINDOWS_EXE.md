...
v0.6.71 adds Windows executable packaging support.
...
## v0.6.71 config/runtime-file fix

PyInstaller one-folder builds may place bundled data under `_internal`.
The launcher expects writable files next to the `.exe`, so v0.6.71 adds two protections:
...
## v0.6.71 self-update note

For executable releases, publish the built folder as:

```text
StoneLightLauncher_v0_6_71_Windows.zip
```
...
## v0.6.71 update ZIP layout recommendation
