...
APP_TITLE = "StoneLight Launcher v0.6.67"
...
UI_TAB_BUTTON_HEIGHT = 32
UI_FOLDER_BUTTON_HEIGHT = 32
...
def apply_window_icon(window):
    try:
        icon_path = ROOT / "assets" / "stonelight_launcher.ico"
        if icon_path.exists():
...
def tr_update_error(text) -> str:
    ...
    match = re.fullmatch(r"В репозитории (.+) не найден latest release\.", text)
    if match:
        return tr("В репозитории {repo} не найден latest release.").format(repo=match.group(1))

    match = re.fullmatch(r"Архив обновления не найден: (.+)", text)
    if match:
        return tr("Архив обновления не найден: {path}").format(path=match.group(1))

    match = re.fullmatch(r"Не удалось проверить обновления StoneLight: (.+)", text)
    if match:
        return tr("Не удалось проверить обновления StoneLight: {error}").format(error=match.group(1))

    match = re.fullmatch(r"ОШИБКА: (.+)", text)
    if match:
...
def attach_button_hover_effect(button, normal_height: int = UI_BUTTON_HEIGHT, hover_delta: int = 0):
    ...
    v0.6.67: no size changes on hover. Changing button height makes Tk grid recalculate
...
def normalize_theme(theme: str | None) -> str:
    ...
    aliases = {
        ...
        "minimal": "laconic",
        "лаконичная": "laconic",
        "laconic_light": "laconic_light",
        "laconic light": "laconic_light",
        "лаконичная светлая": "laconic_light",
        "лаконічна світла": "laconic_light",
        "лаконічна": "laconic",
        "лаконикалық": "laconic",
        "neon": "neon",
        "неон": "neon",
...
def apply_segmented_tab_style(segmented):
    try:
        ...
        segmented.configure(height=UI_TAB_BUTTON_HEIGHT)
    except Exception:
        pass
...
def style_custom_tab_button(button, active: bool):
    try:
        ...
        button.configure(height=UI_TAB_BUTTON_HEIGHT, corner_radius=UI_BUTTON_CORNER_RADIUS)
    except Exception:
        pass
...
def style_folder_nav_button(button, active: bool):
    try:
        if active:
            button.configure(
                fg_color=theme_pair("accent"),
        ...
        button.configure(height=UI_FOLDER_BUTTON_HEIGHT, corner_radius=UI_BUTTON_CORNER_RADIUS)
    except Exception:
        pass
...
def style_kwargs(cls_name: str, kwargs: dict) -> dict:
    ...
    if cls_name in ("CTkFrame", "CTkToplevel"):
        ...
        kwargs.setdefault("corner_radius", UI_BUTTON_CORNER_RADIUS)
        kwargs.setdefault("height", UI_BUTTON_HEIGHT)
    ...
    elif kwargs.get("text_color") == "#ffffff" and kwargs.get("fg_color") != theme_pair("danger"):
...
def patch_i18n_widgets():
    def patch_text_widget(cls):
        ...
        def patched_configure(self, *args, **kwargs):
            ...
            # Do not inject default theme colors when configure() only changes
            # runtime state or geometry/layout properties. Otherwise compacting
            # buttons with height/corner_radius overwrites danger/warning colors.
            color_keys = {"fg_color", "hover_color", "text_color", "border_color"}
            theme_affecting_keys = color_keys | {"border_width"}
            geometry_only_keys = {
                "height", "width", "corner_radius", "font",
                "state", "command", "text", "image", "compound", "anchor"
            }

            if cls_name == "CTkButton" and kwargs:
                keys = set(kwargs)
                if not (keys & theme_affecting_keys):
...
class SelectableListDialog(ctk.CTkToplevel):
    def __init__(self, parent, title: str, items: list[str], on_select, filter_placeholder: str = "Фильтр"):
        ...
        self.info_label.grid(row=2, column=0, padx=16, pady=(0, 6), sticky="w")
...
class VersionPickerDialog(ctk.CTkToplevel):
    def __init__(self, parent, initial_version: str = "", include_snapshots: bool = True, on_select=None):
        ...
        self.reload_button = ctk.CTkButton(top, text=icon_text("refresh", "Обновить список"), width=130, command=self.load_versions)
...
class MicrosoftLoginDialog(ctk.CTkToplevel):
    ...
    def __init__(self, app):
        ...
        ).grid(row=5, column=0, columnspan=3, padx=18, pady=(6, 3), sticky="w")
...
"""
    ...
    def open_login(self):
        try:
            ...
            self.stop_callback_server()
            messagebox.showerror("StoneLight Launcher", tr_update_error(str(exc)))
...
class GlobalLaunchSettingsDialog(ctk.CTkToplevel):
    def __init__(self, app):
        ...
        ctk.CTkButton(buttons, text=icon_text("settings", "Сохранить"), command=self.save).grid(row=0, column=0, padx=(0, 8), sticky="ew")
...
class InstanceWindow(ctk.CTkToplevel):
    ...
    def create_custom_tab_button(self, tab_name: str, column: int):
        tab_icon_key = {
            getattr(self, "tab_launch_name", ""): "play",
            getattr(self, "tab_files_name", ""): "folder",
            getattr(self, "tab_settings_name", ""): "settings",
            getattr(self, "tab_console_name", ""): "console",
        }.get(tab_name, "instance")
    ...
    def build_files_tab(self):
        ...
        for i, (folder, label, icon_key) in enumerate(folder_defs):
            button = ctk.CTkButton(
                buttons,
            ...
            style_folder_nav_button(button, active=False)

        self.folder_label = ctk.CTkLabel(tab, text="Выбери папку выше.", anchor="w")
        self.folder_label.grid(row=1, column=0, columnspan=2, padx=16, pady=(6, 3), sticky="ew")
        ...
        bottom.grid(row=3, column=0, columnspan=2, padx=16, pady=(0, 12), sticky="ew")
        bottom.grid_columnconfigure((0, 1), weight=1)

        self.open_selected_folder_button = ctk.CTkButton(bottom, text=icon_text("open", "Открыть выбранную папку"), command=self.open_current_folder)
        self.open_selected_folder_button.grid(row=0, column=0, padx=(0, 8), sticky="ew")

        self.refresh_files_button = ctk.CTkButton(bottom, text=icon_text("refresh", "Обновить список"), command=self.refresh_current_folder)
        self.refresh_files_button.grid(row=0, column=1, padx=(8, 0), sticky="ew")

        self.current_folder = "mods"
        self.update_folder_nav_buttons()
    ...
    def build_settings_tab(self):
        ...
        ctk.CTkLabel(tab, text="Название").grid(row=1, column=0, padx=16, pady=5, sticky="w")
        self.name_entry = ctk.CTkEntry(tab)
        self.name_entry.grid(row=1, column=1, columnspan=2, padx=16, pady=5, sticky="ew")

        ctk.CTkLabel(tab, text="Minecraft").grid(row=2, column=0, padx=16, pady=5, sticky="w")
        self.mc_entry = ctk.CTkEntry(tab)
        self.mc_entry.grid(row=2, column=1, padx=(16, 8), pady=5, sticky="ew")

        self.pick_mc_button = ctk.CTkButton(tab, text="Выбрать", width=110, command=self.pick_settings_version)
        self.pick_mc_button.grid(row=2, column=2, padx=(8, 16), pady=5, sticky="ew")

        ctk.CTkLabel(tab, text="Loader").grid(row=3, column=0, padx=16, pady=5, sticky="w")
        self.loader_combo = ctk.CTkComboBox(tab, values=get_available_mod_loaders(), command=lambda _: self.on_settings_loader_changed())
        self.loader_combo.grid(row=3, column=1, columnspan=2, padx=16, pady=5, sticky="ew")

        ctk.CTkLabel(tab, text="Loader version").grid(row=4, column=0, padx=16, pady=5, sticky="w")
        self.loader_version_combo = ctk.CTkComboBox(tab, values=[""])
        self.loader_version_combo.grid(row=4, column=1, padx=(16, 8), pady=5, sticky="ew")

        self.load_loader_versions_button = ctk.CTkButton(tab, text="Загрузить", width=110, command=self.load_settings_loader_versions)
        self.load_loader_versions_button.grid(row=4, column=2, padx=(8, 16), pady=5, sticky="ew")

        ctk.CTkLabel(tab, text="Forge mode").grid(row=5, column=0, padx=16, pady=5, sticky="w")
        self.forge_mode_combo = ctk.CTkComboBox(tab, values=["auto", "manual"])
        self.forge_mode_combo.grid(row=5, column=1, columnspan=2, padx=16, pady=5, sticky="ew")

        ctk.CTkLabel(tab, text="Java preset").grid(row=6, column=0, padx=16, pady=5, sticky="w")
        self.java_preset_combo = ctk.CTkComboBox(
            tab,
            values=JAVA_PRESET_VALUES,
            command=lambda _: self.on_settings_java_changed()
        )
        self.java_preset_combo.grid(row=6, column=1, columnspan=2, padx=16, pady=5, sticky="ew")

        ctk.CTkLabel(tab, text="Java этой сборки").grid(row=7, column=0, padx=16, pady=5, sticky="w")
        java_instance_row = ctk.CTkFrame(tab, fg_color="transparent")
        java_instance_row.grid(row=7, column=1, columnspan=2, padx=16, pady=5, sticky="ew")
        ...
        java_buttons.grid(row=8, column=1, columnspan=2, padx=16, pady=(0, 4), sticky="ew")
        ...
        self.java_hint_label.grid(row=9, column=1, columnspan=2, padx=16, pady=(0, 4), sticky="w")

        ctk.CTkLabel(tab, text="Server IP").grid(row=10, column=0, padx=16, pady=5, sticky="w")
        self.server_entry = ctk.CTkEntry(tab)
        self.server_entry.grid(row=10, column=1, columnspan=2, padx=16, pady=5, sticky="ew")
        ...
        self.ensure_server_check.grid(row=11, column=1, columnspan=2, padx=16, pady=5, sticky="w")

        self.save_settings_button = ctk.CTkButton(tab, text="Сохранить настройки сборки", command=self.save_instance_settings)
        self.save_settings_button.grid(row=0, column=2, padx=(8, 16), pady=(14, 6), sticky="ew")

        self.settings_note = ctk.CTkLabel(tab, text="", text_color="#bdbdbd", wraplength=740, justify="left")
        self.settings_note.grid(row=13, column=0, columnspan=3, padx=16, pady=5, sticky="w")
    ...
    def browse_instance_java(self):
        ...
        if not is_manual_java_preset_value(preset):
            self.refresh_instance_java_manual_controls()
            return
    ...
    def on_check_updates(self, auto: bool = False):
        ...
        def wrapper():
            try:
                ...
                if auto:
                    self.ui_queue.put(("log", tr("Автопроверка обновлений не удалась: {error}").format(error=str(exc))))
                else:
    ...
    def handle_updates_result(self, launcher_info: dict, official_info: dict, auto: bool = False):
        ...
        launcher_warning = launcher_info.get("warning")
        if launcher_warning:
            self.append_log(tr_update_error(launcher_warning))
        ...
        if launcher_update:
            message = tr(
                ...
                latest=launcher_info.get("latest_version"),
                current=launcher_info.get("current_version"),
            )
            if not auto:
                if messagebox.askyesno("StoneLight Launcher", message):
                    self.download_launcher_update_worker(launcher_info)
                    return
            else:
                self.append_log(tr("Доступна новая версия лаунчера: {version}").format(version=launcher_info.get("latest_version")))
        ...
        if official_update:
            ...
            if (not auto) and messagebox.askyesno("StoneLight Launcher", tr(message)):
                ...
                self.append_log(tr("Доступно обновление официальной StoneLight-сборки."))
    ...
    def apply_official_update_and_run(self, official_info: dict):
        try:
            ...
            self.append_log(tr("Метаданные официальной сборки обновлены. Запускаю обновление сборки..."))
            self.on_update()
        except Exception as exc:
            messagebox.showerror("StoneLight Launcher", tr_update_error(str(exc)))
            self.append_log(tr_update_error("ОШИБКА: " + str(exc)))
    ...
    def on_update(self):
        self.append_console(tr("Запускаю локальную установку/обновление сборки: {name}").format(
    ...
    def update_folder_nav_buttons(self):
        for name, button in getattr(self, "folder_buttons", {}).items():
            style_folder_nav_button(button, active=(name == getattr(self, "current_folder", None)))
    ...
    def on_java_preset_changed(self):
        ...
        if hasattr(self, "instance_java_entry"):
            self.instance_java_entry.configure(state="normal" if manual else "disabled")
            if not manual:
                try:
                    self.instance_java_entry.delete(0, "end")
                except Exception:
        ...
        if hasattr(self, "instance_java_button"):
            self.instance_java_button.configure(state="normal" if manual else "disabled")
    ...
    def refresh_instance_java_manual_controls(self):
        ...
        for widget_name in ("instance_java_entry", "instance_java_button", "browse_java_button", "java_browse_button"):
...
class StoneLightLauncherApp(ctk.CTk):
    ...
    def build_ui(self):
        ...
        # v0.6.67: hidden to keep the main screen compact; detailed info is in the instance window.
        ...
        # v0.6.67: hidden to keep the main screen compact; the restriction is enforced when adding offline accounts.
        ...
        # v0.6.67: hidden to keep the Java card compact.
        ...
        self.global_settings_button.grid(row=3, column=0, columnspan=3, padx=16, pady=(8, 12), sticky="ew")
        ...
        self.open_log_button.grid(row=2, column=0, padx=10, pady=(3, 6), sticky="ew")
        ...
        # v0.6.67: watermark removed from the main status card; the logo remains in the header and splash screen.
        ...
        self.log_box.insert("end", tr_service("Добро пожаловать в StoneLight Launcher v0.6.67") + "\n")
    ...
    def find_instance_by_ui_label(self, label: str):
        """Find instance by either raw or localized ComboBox label."""
        label = str(label or "")
        instance = find_instance_by_label(self.instances_data, label)
        if instance:
            return instance

        for inst in self.instances_data.get("instances", []):
            raw_label = instance_label(inst)
            try:
                localized_label = tr(raw_label)
            except Exception:
                localized_label = raw_label
            if label == localized_label or label == raw_label:
                return inst
    ...
    def refresh_instances_ui(self):
        ...
        try:
            self.instance_combo.set(tr(selected_label))
        except Exception:
    ...
    def get_selected_instance(self, silent=False):
        ...
        instance = self.find_instance_by_ui_label(label)
        if instance:
            return instance
    ...
    def on_instance_selected(self, label: str = ""):
        ...
        if not instance:
            try:
                instance = self.find_instance_by_ui_label(self.instance_combo.get())
            except Exception:
                instance = None
        if not instance:
            self.refresh_header()
    ...
    def on_check_updates(self, auto: bool = False):
        ...
        def wrapper():
            try:
                ...
                if self.is_official_stonelight_instance(selected_instance):
                    official_info = check_official_modpack_update(cfg)
                else:
                    official_info = {
    ...
    def handle_updates_result(self, launcher_info: dict, official_info: dict, auto: bool = False):
        ...
        official_warning = official_info.get("warning")
        if official_warning:
            self.append_log(tr_update_error(official_warning))

        if official_info.get("skipped") and not auto:
            self.append_log(tr("Проверка StoneLight-сборки пропущена: выбрана не официальная сборка."))
    ...
    def official_update_message(self, official_info: dict, suffix: str = "") -> str:
        ...
        return tr(
            ...
            "Minecraft: {latest}\n\n"
            "{suffix}"
        ).format(asset=asset_name, latest=latest, suffix=suffix)
    ...
    def apply_official_update_metadata(self, official_info: dict, select_official: bool = True):
        ...
        self.append_log(tr("Официальная сборка StoneLight обновлена до {latest}.").format(latest=latest))
        return official
    ...
    def is_official_stonelight_instance(self, instance: dict | None) -> bool:
        if not instance:
            return False
        return bool(instance.get("official")) and str(instance.get("id", "")).lower() == "stonelight"

    def should_check_official_before_launch(self, instance: dict | None) -> bool:
        if not self.is_official_stonelight_instance(instance):
            return False
    ...
    def handle_official_play_update_result(self, official_info: dict):
        if official_info.get("warning"):
            self.append_log(tr_update_error(official_info.get("warning")))

        if not official_info.get("has_update", False):
            self.append_log(tr("Обновлений StoneLight перед запуском не найдено."))
        ...
        message = self.official_update_message(
            official_info,
            suffix=tr("Обновить StoneLight до {latest} перед запуском?").format(latest=latest)
        )

        if messagebox.askyesno("StoneLight Launcher", tr(message)):
            try:
                self.apply_official_update_metadata(official_info, select_official=True)
                self.append_log(tr("Обновление StoneLight применено. Запускаю новую версию..."))
            ...
            if self.config.get("official_modpack_offer_skip_update", True):
                if messagebox.askyesno(
                    ...
                    self.append_log(tr("Запуск StoneLight отменён пользователем."))
            else:
                self.append_log(tr("Запуск StoneLight отменён: обновление не применено."))

    def handle_official_play_update_check_failed(self, error_text: str):
        self.append_log(tr("Не удалось проверить обновления StoneLight: {error}").format(error=str(error_text)))
        if messagebox.askyesno(
            "StoneLight Launcher",
            ...
            self.append_log(tr("Запуск StoneLight отменён."))
    ...
    def on_update(self):
        ...
        # and by the official StoneLight pre-launch update flow.
        self.start_worker(
            lambda core, username, ram_mb, java, account: core.update_only(java, force_download=True),
            "Сборка обновлена/установлена."
        )
